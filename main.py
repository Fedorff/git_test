...
TOKEN = "7033871397:AAFndmO6Cvgr6eKLttFHlJpd6PId13bLXOo"
bot = TeleBot(TOKEN)
game = False
night = False
...
def kill(message):
    ...
    if night and message.from_user.first_name in mafia_usernames:
        ...
        if voted:
            bot.send_message(message.chat.id, 'Ваша цель установлена')
            return
        bot.send_message(message.chat.id, 'У вас больше нет права убивать')
        return
...
def game_on(message):
    if not game:
        bot.send_message(message.chat.id, text='Если хотите поиграть, напишите "готов играть" в лс')
