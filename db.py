...
def check_winner():
    con = sqlite3.connect("db.db")
    cur = con.cursor()
    sql_mafia = f'SELECT COUNT(*) FROM players WHERE role = "mafia" and dead = 0'
    sql_citizen = f'SELECT COUNT(*) FROM players WHERE role = "citizen" and dead = 0'
    cur.execute(sql_mafia)
    mafia_alive = cur.fetchone()[0]
    cur.execute(sql_citizen)
